...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model, mg=False):
        ...
        self.difflr = config['difflr']
        self.diffmmlr = config['diffmmlr']
        if self.dataset == 'baby':
            self.delay_epoch = 30
        elif self.dataset == 'sports':
            self.delay_epoch = 40
        else:
            self.delay_epoch = 40 # TODO
        self.optimizer = self._build_optimizer()

        #fac = lambda epoch: 1 ** (epoch / 50)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, writer=None):
        ...
        for batch_idx, interaction in enumerate(train_data):
            ...
            if self.model_name == 'AMUR':
                losses = loss_func(interaction, epoch_idx)
            else:
                losses = loss_func(interaction)
                
            if isinstance(losses, tuple):
                loss = sum(losses) # multi-loss summation
            ...
            if self.mg and batch_idx % self.beta == 0: # 0
                ...
                loss.backward()
                # print(self.model.modal_weight.grad)
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True, writer=None): # tb
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            writer.add_scalar("Training Loss", self.train_loss_dict[epoch_idx], epoch_idx) # tb
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                writer.add_scalar("Testing acc:", test_score, epoch_idx) # tb
                if verbose:
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        for batch_idx, batched_data in enumerate(eval_data):
            ...
            # mask out pos items
            scores[masked_items[0], masked_items[1]] = -1e10 # train mask
            # rank and get top-k
